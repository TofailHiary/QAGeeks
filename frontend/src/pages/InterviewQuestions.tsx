  ...
  return (
    <PageLayout title="QA Interview Questions" subtitle="Common interview questions for Quality Assurance roles">
      <div className="max-w-6xl mx-auto py-12 px-4 space-y-12">
        <p className="text-gray-700 leading-relaxed text-lg text-center max-w-3xl mx-auto mb-12"> {/* Reverted to darker text for visibility on light background */}
          Explore a comprehensive list of categorized QA interview questions to help you prepare effectively and confidently.
        </p>

        <Accordion type="multiple" className="space-y-6"> {/* Increased spacing */}
          {interviewQuestions.map((category) => (
            <AccordionItem
              key={category.category}
              value={category.category}
              className="border border-purple-300/30 bg-white/10 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-purple-500/20" // Slightly increased opacity
            >
              {/* Changed text color to purple, removed font-semibold and text-xl to simplify */}
              <AccordionTrigger className="px-6 py-5 text-left text-white font-bold text-2xl rounded-lg shadow-md bg-gradient-to-r from-[#00A2FF] to-[#9C27FF] hover:brightness-110 transition-all duration-300 [&>svg]:text-yellow-300 [&>svg]:stroke-[2.5]">
                {category.category}
                {/* Default arrow from shadcn/ui should render correctly now */}
              </AccordionTrigger>
              <AccordionContent className="bg-white/95 px-6 pt-4 pb-6 text-gray-800"> {/* Increased opacity, adjusted padding */}
                <ul className="space-y-5"> {/* Increased spacing between questions */}
                  {category.questions.map((q, index) => (
                    <li key={index} className="border-l-4 border-blue-400 pl-4"> {/* Left border accent */}
                      {typeof q === "string" ? (
                        <span className="italic text-gray-600">{q} (Answer pending)</span> // Indicate unanswered
                      ) : (
                        <div>
                          <strong className="block text-lg font-medium text-blue-800 mb-1.5">{q.question}</strong> {/* Adjusted question style */}
                          <span className="block text-base leading-relaxed text-gray-700">{q.answer}</span> {/* Adjusted answer style */}
                        </div>
                      )}
                    </li>
                  ))}
